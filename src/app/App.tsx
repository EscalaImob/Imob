...
type PageKey = "platformAdmin" | "overview" | "goals" | "clients" | "leads" | "buyersFunnel" | "captureFunnel" | "opportunityDetail" | "properties" | "propertyEditor" | "publications" | "authorizations" | "authorizationEditor" | "contracts" | "contractEditor" | "inspections" | "inspectionEditor" | "finance" | "reports" | "settings" | "tasks" | "agenda" | "visits";
...
const navigation: NavGroup[] = [
  { label: "Meu painel", items: [
    ...
    { label: "Metas de vendas", path: "/app/metas/", icon: TargetIcon, permission: "sales.goals.read" },
  ] },
...
function currentPage(): { key: PageKey; group: string; label: string; title: string } {
  ...
  if (path === "/app/metas/") return { key: "goals", group: "Meu painel", label: "Metas de vendas", title: "Metas de vendas" };
...
function OverviewPage({ bootstrap }: { bootstrap: AppBootstrapResult }) {
  ...
  const pipelineMaximum = Math.max(1, ...pipelineStages.map((stage) => stage.opportunities.length));
  const pipelinePie = pipelineStages.reduce<{cursor:number;parts:string[]}>((chart,stage) => { const size = funnelTotal ? stage.opportunities.length / funnelTotal * 100 : 0; chart.parts.push(`${stage.color} ${chart.cursor}% ${chart.cursor + size}%`); chart.cursor += size; return chart; }, { cursor: 0, parts: [] }).parts.join(", ");
  const pipelinePoints = pipelineStages.map((stage,index) => `${pipelineStages.length === 1 ? 50 : index * 100 / (pipelineStages.length - 1)},${94 - stage.opportunities.length / pipelineMaximum * 82}`).join(" ");
  ...
  return (
    <>
      ...
      <section className="app-dashboard-grid">
        <article className="app-dashboard-panel app-dashboard-panel--wide"><header><div><FunnelIcon /><strong>Pipeline comercial</strong></div><a href="/app/funis/compradores/">Abrir funis</a></header>{pipelineStages.length === 0 ? <div className="app-dashboard-panel__empty"><FunnelIcon /><h2>Nenhuma oportunidade ativa</h2><p>Crie oportunidades nos funis de compradores e captação.</p></div> : <div className="app-dashboard-pipeline-layout"><div className="app-dashboard-pipeline">{pipelineStages.map((stage) => <article key={`${stage.funnelName}-${stage.id}`}><span style={{ background: stage.color }} /><div><strong>{stage.name}</strong><small>{stage.funnelName}</small></div><b>{stage.opportunities.length}</b></article>)}</div><div className="app-dashboard-pipeline-charts" aria-label="Gráficos do pipeline"><article><h3>Distribuição</h3><div className="app-pipeline-donut" style={{background:pipelinePie?`conic-gradient(${pipelinePie})`:undefined}}><strong>{funnelTotal}</strong><small>negócios</small></div></article><article><h3>Volume por etapa</h3><div className="app-pipeline-bars">{pipelineStages.map(stage=><span key={stage.id} title={`${stage.name}: ${stage.opportunities.length}`} style={{height:`${Math.max(5,stage.opportunities.length/pipelineMaximum*100)}%`,background:stage.color}}/>)}</div></article><article><h3>Evolução pelas etapas</h3><svg className="app-pipeline-line" viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="Linha de volume das etapas"><polyline points={pipelinePoints}/></svg></article></div></div>}</article>
