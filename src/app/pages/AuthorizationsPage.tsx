...
import {
  ...
  type PortfolioAuthorizationStatus,
  type PortfolioAuthorizationType,
...
const statusLabels: Record<PortfolioAuthorizationStatus, string> = {
  draft: "Rascunho",
...
const typeLabels: Record<PortfolioAuthorizationType, string> = {
  sale: "Venda",
  rent: "Locação",
  sale_rent: "Venda ou locação",
  capture: "Captação",
};
...
export function AuthorizationsPage({ organizationId, canCreate, canUpdate }: Props) {
  ...
  const query = useMemo(
    ...
    [search, status, type, expiresFrom, expiresTo, propertyId, page],
  );
  async function removeAuthorization(id: string) {
    if (!globalThis.confirm("Excluir esta autorização? Ela será cancelada e o histórico será preservado.")) return;
  ...
  useEffect(() => {
    const handle = globalThis.setTimeout(() => {
      setLoading(true);
      setError(null);
      void listAuthorizations(organizationId, query)
        .then(setResult)
        .catch((loadError) =>
          setError(
    ...
    <>
      <section className="app-page-heading">
        <div>
          ...
          <h1>Autorizações</h1>
          <p>
            Controle condições de comercialização, vigência, responsáveis e
            vencimentos.
          </p>
        </div>
        {canCreate && (
          <a className="app-primary-button" href={newHref}>
            + Nova autorização
          </a>
        )}
      </section>
      {propertyId && (
        <div className="app-context-banner">
          <DocumentIcon />
          <div>
            <strong>Filtro por imóvel ativo</strong>
            <p>
              A listagem mostra somente autorizações do imóvel aberto
              anteriormente.
            </p>
          </div>
          <a href="/app/autorizacoes/">Ver todas</a>
        </div>
      )}
      {error && <div className="app-inline-error">{error}</div>}
      <section className="app-metrics app-metrics--compact">
        {[
          ["Total autorizações", result?.total ?? 0],
          ["Vigentes", result?.active ?? 0],
          ["A vencer (30 dias)", result?.expiring ?? 0],
          ["Vencidas", result?.expired ?? 0],
        ].map(([label, value]) => (
      ...
      <section className="app-filter-panel app-authorization-filters">
        <div className="app-search-field">
          <SearchIcon />
          <input
            placeholder="Imóvel, código ou proprietário..."
            value={search}
            onChange={(event) => {
        ...
        <label>
          ...
          <input
            ...
            value={expiresFrom}
            onChange={(event) => {
              setExpiresFrom(event.target.value);
              setPage(1);
            }}
          />
        </label>
        <label>
          ...
          <input
            ...
            value={expiresTo}
            onChange={(event) => {
              setExpiresTo(event.target.value);
              setPage(1);
            }}
          />
        </label>
        <select
          ...
          value={status}
          onChange={(event) => {
            setStatus(event.target.value as PortfolioAuthorizationStatus | "");
            setPage(1);
          }}
        >
        ...
        <select
          ...
          value={type}
          onChange={(event) => {
            setType(event.target.value as PortfolioAuthorizationType | "");
            setPage(1);
          }}
        >
        ...
        {(search || status || type || expiresFrom || expiresTo) && (
          <button
      ...
      <section className="app-data-card">
        {loading ? (
          ...
          <div className="app-table-empty">
            <DocumentIcon />
            <h2>Nenhuma autorização encontrada</h2>
            <p>Crie uma autorização ou ajuste os filtros selecionados.</p>
            {canCreate && (
              <a className="app-primary-button" href={newHref}>
          ...
          <>
            <div className="app-authorization-table app-action-table">
              <header>
                <span>Imóvel</span>
              ...
              {result.items.map((item) => (
                ...
                >
                  ...
                  <span>{item.primaryOwner?.name ?? "—"}</span>
                  <span>{typeLabels[item.type]}</span>
                  <span>{item.exclusive ? "Sim" : "Não"}</span>
                  <span>{percent(item.commissionPercent)}</span>
                  <span>{dateLabel(item.startsAt)}</span>
                  <span>{dateLabel(item.endsAt)}</span>
                  <span>
                    <em
                      className={`app-status-pill authorization-${item.status}`}
                    >
            ...
            <div className="app-pagination">
              <span>
                {result.totalItems}{" "}
                {result.totalItems === 1 ? "autorização" : "autorizações"}
              </span>
              <div>
                <button
                  type="button"
