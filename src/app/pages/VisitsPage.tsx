...
import { listVisits, type VisitListItem, type VisitStatus, type VisitView } from "../../services/visitsApi";
...
export function VisitsPage({ organizationId, canCreate, canUpdate, canReadContacts, canReadOpportunities, canReadProperties }: Props) {
  ...
  return <>
    <section className="app-module-header"><div><span className="app-section-eyebrow">Produtividade</span><h1>Gestão de visitas</h1><p>Agende, confirme e acompanhe visitas com histórico conectado ao CRM.</p></div>{canCreate && canReadContacts && <button className="app-primary-button" type="button" onClick={() => setModalVisit(null)}>+ Nova visita</button>}</section>
    ...
    <section className="app-productivity-toolbar app-visits-toolbar">
      ...
      <div className="app-productivity-search"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar cliente, visita ou local..." /></div>
    ...
    {loading ? <div className="app-list-loading">Carregando visitas...</div> : !result || result.items.length === 0 ? <section className="app-productivity-empty app-visits-empty"><PinIcon /><h2>Nenhuma visita encontrada</h2><p>{canCreate && canReadContacts ? "Agende a primeira visita ou ajuste os filtros." : "Não há visitas disponíveis para os filtros atuais."}</p></section> : <section className="app-visits-list"><header><span>Visita</span><span>Cliente</span><span>Data e horário</span><span>Responsável</span><span>Status</span></header>{result.items.map((visit) => <button type="button" key={visit.id} onClick={() => { if (canUpdate) setModalVisit(visit); }} aria-disabled={!canUpdate}><span><strong>{visit.title}</strong><small>{visit.location}{visit.property ? ` · ${visit.property.internalCode} — ${visit.property.title}` : visit.opportunity ? ` · ${visit.opportunity.title}` : ""}</small></span><span>{visit.contact.name}</span><span><strong>{formatDate(visit.startsAt)}</strong><small>até {new Intl.DateTimeFormat("pt-BR", { hour: "2-digit", minute: "2-digit" }).format(new Date(visit.endsAt))}</small></span><span>{visit.responsible?.displayName ?? "—"}</span><span><em className={`app-visit-status status-${visit.status}`}>{statusLabels[visit.status]}</em>{visit.status === "completed" && visit.feedbackRating ? <small>{"★".repeat(visit.feedbackRating)}</small> : null}</span></button>)}</section>}
