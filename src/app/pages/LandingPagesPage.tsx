...
export function LandingPagesPage({ organizationId, canManage }: { organizationId: string; canManage: boolean }) {
  ...
  useEffect(()=>{if(localPreviewMode)return;let active=true;void listProperties(organizationId,{status:"active",pageSize:100}).then(result=>{if(active)setCatalogProperties(result.items.filter(item=>!item.archivedAt));}).catch(()=>{if(active)setCatalogProperties([]);});return()=>{active=false;};},[organizationId]);
